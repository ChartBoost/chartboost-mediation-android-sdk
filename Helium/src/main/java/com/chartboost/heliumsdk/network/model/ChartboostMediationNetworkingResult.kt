...
import com.chartboost.heliumsdk.network.NetworkErrorTransformer
...
sealed class ChartboostMediationNetworkingResult<out T> {
    ...
    class Success<out T> @PublishedApi internal constructor(
        val httpCode: Int,
        val headers: Headers,
        val body: T?
    ) : ChartboostMediationNetworkingResult<T>()

    class JsonParsingFailure @PublishedApi internal constructor(
        val code: Int,
        val headers: Headers,
    ...
    class Failure @PublishedApi internal constructor(
        val code: Int,
        val headers: Headers?,
        val error: ChartboostMediationError,
            ...
            return when {
                response.isSuccessful->
                    ...
                    Failure(
                        code = response.code(),
                        headers = response.headers(),
                        error = error?.chartboostMediationError
