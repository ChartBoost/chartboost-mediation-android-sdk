...
fun ChartboostMediationNetworkingTest.`verify result for sdk_init success with new config`() = runTest {
    ...
    launch {
        ...
        val expectedResult = HeliumJson.decodeFromString(
...
fun ChartboostMediationNetworkingTest.`verify result for sdk_init success with no new config`() = runTest {
...
fun ChartboostMediationNetworkingTest.`verify result for sdk_init failure due to JSON parsing`() = runTest {
    val expectedResponseHttpCode = 200
    MockResponse()
        .setResponseCode(expectedResponseHttpCode)
        .setHeader(ChartboostMediationNetworking.INIT_HASH_HEADER_KEY, ChartboostMediationNetworkingTest.INIT_HASH_NEW)
...
fun ChartboostMediationNetworkingTest.`verify result for sdk_init failure due to http code`() = runTest {
