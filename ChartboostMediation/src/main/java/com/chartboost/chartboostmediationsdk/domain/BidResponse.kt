...
data class BidResponse(
    ...
    @SerialName(CHARTBOOST_MEDIATION_BID_ID_KEY)
    val chartboostMediationBidId: String,
) : Comparable<BidResponse> {
    val isMediation: Boolean =
        bidInfoArray.firstOrNull()?.id.equals(MEDIATION_KEY, ignoreCase = true)

    val partnerPlacement = bidInfoArray.firstOrNull()?.ext?.partnerPlacement ?: ""
    ...
    val partnerSettings: MutableMap<String, String> =
        mutableMapOf<String, String>().apply {
            bidInfoArray.firstOrNull()?.ext?.bidderInfo?.jsonObject?.getValue(HELIUM_KEY)?.jsonObject?.let {
                it.keys.forEach { key ->
                    val value = ChartboostMediationJson.decodeFromJsonElement<String>(it.getValue(key))
                    put(key, value)
                }
            }
        }
    ...
    companion object {
        ...
        private const val CHARTBOOST_MEDIATION_BID_ID_KEY = "helium_bid_id"
    }
}
