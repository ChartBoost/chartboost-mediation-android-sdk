...
class AdRefresher(
    ...
    private val refreshRunnable =
        Runnable {
            LogController.i("Chartboost Mediation AdRefresherCallback onAdNeedsRefreshing.")
    ...
    private fun scheduleNextRefresh() {
        ...
        // To enable debug logging set ChartboostMediationSdk.setDebugMode(true)
        LogController.i(
            "Chartboost Mediation AdRefresherCallback start. Current refresh rate at: " +
    ...
    fun cancel(resetTimer: Boolean = true) {
        LogController.i("Chartboost Mediation AdRefresherCallback cancel.")
        isResumed = false
        handler.removeCallbacks(refreshRunnable)
        if (resetTimer) {
            totalTimePausedMs = 0
        ...
        LogController.i("Chartboost Mediation AdRefresherCallback reset timer. Viewed for: $totalTimePausedMs")
    }
}
