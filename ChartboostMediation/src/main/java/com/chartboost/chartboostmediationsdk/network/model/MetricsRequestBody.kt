...
class MetricsData private constructor(
    ...
    val chartboostMediationError: String? = null,
    ...
    constructor(metrics: Metrics) : this(
        ...
        end = if (didPartnerTimeOut(metrics.chartboostMediationError?.name)) null else metrics.end,
        duration = if (didPartnerTimeOut(metrics.chartboostMediationError?.name) || metrics.duration?.sign != 1) null else metrics.duration,
        isSuccess = metrics.isSuccess,
        chartboostMediationError = metrics.chartboostMediationError?.serverErrorName,
    ...
    companion object {
        ...
        private fun didPartnerTimeOut(errorName: String?) =
            errorName.equals(ChartboostMediationError.InitializationError.Timeout.name) ||
                errorName.equals(ChartboostMediationError.PrebidError.Timeout.name) ||
                errorName.equals(ChartboostMediationError.LoadError.AdRequestTimeout.name) ||
                errorName.equals(ChartboostMediationError.ShowError.Timeout.name) ||
                errorName.equals(ChartboostMediationError.InitializationError.Timeout.name)
    }
}
