...
class InterstitialApiTest {
    ...
    private val interstitialMock = mockk<Interstitial>()
    ...
    private val interstitialApiKitkat =
        InterstitialApi(
            adUnitLoaderMock,
            adUnitRendererMock,
            uiPosterMock,
    ...
    fun `cache but session not started cause sdk below Lollipop `() {
        every { interstitialCallbackMock.onAdLoaded(any(), any()) } just runs
        interstitialApiKitkat.cache(interstitialMock, interstitialCallbackMock)
        verify(exactly = 1) {
            uiPosterMock(capture(lambdaCaptor))
        }
        lambdaCaptor.captured.invoke()
    ...
    fun `cache but session not started cause invalid location `() {
        every { interstitialMock.location } returns ""
        every { interstitialCallbackMock.onAdLoaded(any(), any()) } just runs
        interstitialApi.cache(interstitialMock, interstitialCallbackMock)
    ...
    fun `on cache success`() {
        interstitialApi.cache(interstitialMock, interstitialCallbackMock)
        interstitialApi.onCacheSuccess("1111", TrackingEventName.Cache.START)
        verify(exactly = 1) {
            adApiCallbackSenderMock.sendCacheCallbackInMainThread(
                any(),
                eq(null),
                eq(interstitialMock),
                eq(interstitialCallbackMock),
            )
        }
    }
    ...
    @Test
    fun `interstitial calls the callback onAdDismiss`() {
        interstitialApi.cache(interstitialMock, interstitialCallbackMock)
        interstitialApi.onImpressionDismissed("test")
        verify(exactly = 1) {
            adApiCallbackSenderMock.sendDismissCallbackOnMainThread(
                eq("test"),
                eq(interstitialMock),
    ...
    fun `on ad show failure after show success is not sent`() {
        ...
        interstitialApi.cache(interstitialMock, interstitialCallbackMock)
        interstitialApi.show(interstitialMock, interstitialCallbackMock)
        interstitialApi.onShowSuccess("test")
        interstitialApi.onShowFailure("test", CBError.Impression.ERROR_CREATING_VIEW)
        verify(exactly = 1) {
            adApiCallbackSenderMock.sendShowCallbackInMainThread(
                eq("test"),
                eq(null),
    ...
    fun `call on impression success`() {
        ...
        interstitialApi.cache(interstitialMock, interstitialCallbackMock)
        interstitialApi.onImpressionSuccess(impressionId)
        verify(exactly = 1) {
            adApiCallbackSenderMock.sendImpressionRecordedCallbackOnMainThread(
                impressionId,
                interstitialMock,
                interstitialCallbackMock,
            )
        }
    }
    ...
    private fun onCacheErrorTest(
        ...
        val interstitialSlot = CapturingSlot<Interstitial>()
        val interstitialCallbackSlot = CapturingSlot<InterstitialCallback>()

        val uiMock = mockk<FakeUiPoster>()
        val localInterstitialMock = mockk<Interstitial>()
        val localInterstitialCallbackMock = mockk<InterstitialCallback>()
        ...
        every { localInterstitialMock.location } returns "test"
        val api =
            InterstitialApi(
                adUnitLoaderMock,
                adUnitRendererMock,
        ...
        api.cache(localInterstitialMock, localInterstitialCallbackMock)
        ...
        verify(exactly = 1) {
            localAdApiCallbackSenderMock.sendCacheCallbackInMainThread(
                ...
                capture(interstitialSlot),
                capture(interstitialCallbackSlot),
            )
        }
