...
class VisibilityTracker(
     ...
     */
    fun interface VisibilityTrackerListener {
        /**
    ...
    private fun setViewTreeObserver() {
        try {
            ...
            Logger.d("Exception when accessing view tree observer.")
        }

        val viewTreeObserver =
            getTopmostView(weakActivity.get(), trackedView)?.viewTreeObserver ?: return
        if (!viewTreeObserver.isAlive) {
            Logger.e("Unable to set ViewTreeObserver since it is not alive")
            return
        }
        ...
        job =
            CoroutineScope(Main).launch(
                CoroutineExceptionHandler { _, throwable ->
                    Logger.d("Visibility check ran into a problem: $throwable")
                },
            ) {
                while (isActive) {
    ...
    private fun isViewVisible(): Boolean {
        ...
        val widthDips = pixelsToIntDips(cachedRect.width(), trackedView.context)
        val heightDips = pixelsToIntDips(cachedRect.height(), trackedView.context)
        return widthDips * heightDips >= minVisibleDips
    }

    private fun pixelsToIntDips(
        length: Int,
        context: Context,
    ): Int {
        return (length * context.resources.displayMetrics.density).roundToInt()
    }
}
